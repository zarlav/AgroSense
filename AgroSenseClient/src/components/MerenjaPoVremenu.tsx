...
export default function MerenjaPoDanu() {
    ...
    const fetchMerenjaPoDanu = () => {
        if(!idSenzora) { setError("Unesi ID senzora!"); return; }
        if(!dan) { setError("Unesite datum u formatu yyyy-mm-dd"); return; }
        if(!vremeOd) { setError("Unesite vreme u formatu hh:mm"); return; }
        if(!vremeDo) { setError("Unesite vreme u formatu hh:mm"); return; }
        ...
        fetch(`https://localhost:7025/api/Merenja/po_vremenu?senzorId=${idSenzora}&dan=${dan}&vremeOd=${vremeOd}&vremeDo=${vremeDo}`)
            .then(res => { 
                if(!res.ok) throw new Error("Greska pri ucitavanju merenja po vremenu!");
                return res.json();
            })
